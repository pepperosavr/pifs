...
if mode == "Режим истории":
    ...
    st.subheader("Доходность (накопленная, %): 1 паи, купленныи в разные периоды")

# 1) База цен: одна цена close на фонд/дату (схлопываем повторы)
    price_base = df_sel.dropna(subset=["close"]).copy()
    price_base = (
        price_base.sort_values(["label", "tradedate"])
                  .groupby(["label", "fund", "isin", "tradedate"], as_index=False)
                  .agg(close=("close", "last"))
    )

# ограничиваем сверху выбраннои конечной датой (чтобы горизонты якорились на end_date)
    price_base = price_base[price_base["tradedate"] <= end_date].copy()
    ...
    # базовая дата и базовая цена (первая точка внутри горизонта для каждого фонда)
    ...
    # накопленная доходность в %
        d["ret_pct"] = (d["close"] / d["base_close"] - 1.0) * 100.0
        return d

# 2) Горизонты: "все время", "1 год", "3 года" от выбранной end_date
    ...
    # (опционально) краткая таблица итоговой доходности на end_date
        last_points = (
        ...
        out = out.rename(columns={
            "fund": "Фонд",
            "base_date_str": "База (дата покупки 1 пая)",
        ...
        disp = out.copy()
        disp["Цена на базовую дату, руб"] = disp["Цена на базовую дату, руб"].map(lambda x: "—" if pd.isna(x) else f"{x:,.2f}".replace(",", " "))
        disp["Цена на конечную дату, руб"] = disp["Цена на конечную дату, руб"].map(lambda x: "—" if pd.isna(x) else f"{x:,.2f}".replace(",", " "))
        disp["Доходность, %"] = disp["Доходность, %"].map(lambda x: "—" if pd.isna(x) else f"{x:+.2f}%")

        st.dataframe(disp, use_container_width=True, hide_index=True)

    with tab_all:
        ret_all = build_cum_return(price_base, start_date=None, end_date_=end_date)
        plot_return_tab(ret_all, "Горизонт: все время")

    with tab_1y:
        ret_1y = build_cum_return(price_base, start_date=start_1y, end_date_=end_date)
        plot_return_tab(ret_1y, "Горизонт: 1 год")

    with tab_3y:
        ret_3y = build_cum_return(price_base, start_date=start_3y, end_date_=end_date)
