    ...
    <form method="post" id="portfolio-form">
        ...
        <div class="topbar">
            <div class="title">
                ...
                <?php if ($actualStart !== null && $actualEnd !== null): ?>
                    <div class="meta-note">Фактическое окно общих исторических данных: <?= h($actualStart) ?> — <?= h($actualEnd) ?></div>
                <?php endif; ?>
            </div>
        ...
        <div class="submit-row">
            <button type="submit" class="btn">Пересчитать портфель</button>
        </div>
        <div class="hint">Логика ребалансировки сохранена: при включении MREFTR веса автоматически переходят в модель 34/21/17/13/15, при выключении — в базовую 40/25/20/15/0.</div>
    </form>
    ...
    <?php if ($reOn): ?>
        <div class="insight">
            ...
            <div class="insight-grid">
                <div>📦 MREFTR — индекс МосБиржи, отражающий динамику фондов недвижимости полной доходности.</div>
                <div>📈 Фонды в составе индекса могут обеспечивать регулярные рентные выплаты — источник дохода даже при слабом рынке акций.</div>
